...
- 用户于 2026-09-20 明确要求发布 RC2 安装包与产品页，不要求签名。
- 同步六个 CTA、安装包哈希/大小、preview 更新清单、错误处理说明及版本历史；保留旧资产，不改购买或部署配置。
- 沿用 GitHub main -> Netlify 发布链，仅提交本次相关文件；实机状态切换验收尚未完成，不宣称已完成。
...
- 2026-09-20 用户授权增加并发布 WorkBuddy 对比 FAQ：仅新增首页 FAQ 与导航锚点，附用户提供的远程通道截图，来源文件名保留在 assets；不采用 AI 回答截图作为事实依据。
- 对比以 WorkBuddy 官方接入指南和客户端实际能力为准；不把远程通道说成发信接口，不宣称 WorkBuddy 无法借助其他工具发送；定时任务依赖外部调度，不暗示本客户端自带后台定时服务。
